    ...
    <main className="min-h-screen bg-[#0a0f1e] text-white p-4 md:p-8 font-sans uppercase">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          ...
          <div className="bg-blue-600 p-5 rounded-xl shadow-lg shadow-blue-900/20"><p className="text-[10px] text-blue-100 font-bold mb-1">Saldo</p><h2 className="text-xl font-black text-white">R$ {(totalIncomes - totalExpenses).toLocaleString('pt-BR')}</h2></div>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
          <div className="md:col-span-4 bg-[#111827] p-6 rounded-2xl border border-slate-800">
            <h3 className="text-[10px] font-black text-blue-500 mb-6 tracking-widest uppercase">Novo Lançamento</h3>
            <form onSubmit={handleAddTransaction} className="space-y-4">
              ...
              <button disabled={loading} className="w-full bg-blue-600 py-4 rounded-xl font-black text-xs tracking-widest hover:bg-blue-500 transition-all uppercase">
          ...
          <div className="md:col-span-8 bg-[#111827] p-6 rounded-2xl border border-slate-800">
            <h3 className="text-[10px] font-black text-slate-500 mb-6 italic tracking-widest">Histórico</h3>
            <div className="space-y-2">
              {filtered.map((t) => (
                <div key={t.id} className="flex justify-between items-center p-4 bg-[#0a0f1e]/50 rounded-xl border border-slate-800/50">
                  <div><p className="text-xs font-black text-slate-200">{t.description}</p><p className="text-[9px] text-slate-500 font-bold">{new Date(t.created_at).toLocaleDateString('pt-BR')}</p></div>
                  <span className={`text-xs font-black ${['income', 'ENTRADA'].includes(t.type) ? 'text-emerald-400' : 'text-rose-500'}`}>R$ {t.amount.toLocaleString('pt-BR')}</span>
                </div>
              ))}
            </div>
          </div>
